...
def ddp_all_reduce_mean_std(x: torch.Tensor, dim=0, eps: float = 1e-6) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if ddp_is_initialized():
        ...
        mu /= world
        var /= world
    sd = var.clamp_min(eps).sqrt()
    return mu, sd

def broadcast_scalar(value: float, src: int = 0) -> float:
    if not ddp_is_initialized():
        return value
    t = torch.tensor([value], device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
